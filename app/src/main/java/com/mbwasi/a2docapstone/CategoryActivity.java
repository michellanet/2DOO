        ...
        request.setOnResponseListener(new HttpRequest.OnResponseListener() {
            ...
            public void onResponse(HttpResponse response) {
                if (response.code == HttpResponse.HTTP_OK) {
                    ...
                    try {
                        Log.e(TAG, response.toJSONObject().toString());
                        JSONArray jsonArrayOfCategories = response.toJSONObject().getJSONArray("data");

                        Log.e(TAG, "Data: "+jsonArrayOfCategories.toString());
                        Type listType = new TypeToken<ArrayList<Category>>(){}.getType();
                        List<Category> categoriesList = new Gson().fromJson(jsonArrayOfCategories.toString(), listType);
